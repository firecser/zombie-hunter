...
## 禁止混淆代码 + 副本同步（2026-08-18 用户硬性要求，最高优先级）
- 用户原话："以后你不允许对代码进行混淆。" → **硬约束，优先级高于任何"为上传而加固"的便利**。仓库里 `game.js` 必须始终是**可读多行源码**；禁止混淆/minify 产物进 git。
- 加固由用户自理：用户用「微信开发者工具」自行加固上传 `game.js`，**仓库永远只留可读版**。绝不允许混淆版停留在工作区被提交。
- **副本文件**：`game copy.js` 是 `game.js` 的镜像备份（用户重命名的正式副本文件名，替代旧 `game - 副本.js`）。用途：防单文件丢失信息。
- **标准工作流（每次做功能务必遵守）**：
  1. 编辑 `game.js`（唯一源码，可读版）；
  2. `git commit` 提交 `game.js` 改动；
  3. **同步副本**：`cp game.js "game copy.js"`，并把副本一并 `git add` + 提交，使两者在仓库内始终一致。
  - 即「先提交 game.js，再同步副本并提交」，二者都进 git，避免信息丢失。
- 现状（2026-08-19）：工作区 `game.js` 为用户去混淆的纯可读源码（~14084 行、0 混淆标记）。`game copy.js` 已与 `game.js` 完全一致（字节相同）。
- 版本提醒：该去混淆 `game.js` 含完整天赋树（v1.1.33 起）但**早于 v1.1.40 统御分支/分栏子页、且不含 v1.1.53 失败弹窗「变强/返回」改动**（它是 v1.1.33~v1.1.39 区间某次构建，非最新 HEAD）。基于此源码改动前先知情——若需失败弹窗三按钮改动需重做。

## 难度/心流曲线架构（2026-08-19，改平衡前必读）
- **波次=局内等级耦合**：每清一波经验刚好升 1 级（耦合设计），故 波次 w ≈ 玩家等级 L（w==L）。通关=消灭 20 波（WAVE_COUNT=20）。
- **9 段难度表** `WAVE_TIER_TABLE`（~L114）：`{tier,from,to,hpT,dmgT}`，9 段覆盖波 1-20；`getWaveTier(wave)` 返回该波 tier。怪物强度 = `STAGES.healthMult × tier.hpT`（HP）、`STAGES.damageMult × tier.dmgT`（伤害），在 `updatePendingSpawns`（~L5516）计算。**原 `stage.hpWaveGrow/hpGrow` 指数/时间线性增长已废弃**，tier 表单一主控。
- **技能解锁等级** `reqLevel`（SKILL_DEFS 各分支）：2026-08-19 重排为 **基础分支 5 / 8 / 11 / 14 / 17**（旧 2/3/4/5/6），雷霆一击(单次性 apex) 独立 **Lv18**。互斥分支对保持同档。新技能**类型**获取不受限（Lv1 即可拿），但其**分支威力**按 reqLevel 在 Lv5+ 才解锁——元素基础弹道(level>0 即启用 AoE/冻结/克制)保证 Lv1-4 升级有效。
- **心流对齐**：怪物 tier 跳档(波 5/8/11/13/16/18/20) 与 技能分支洪流(Lv5/8/11/14/17/18) 几乎重合 → 升压时玩家同时获强。均值玩家(r≈8%/级有效成长)全程偏差≈≤23%（玩家领先=偏易，安全方向）；谨慎玩家(r≈6%)后期略难；玻璃大炮(r≥10%)偏易。调平衡就改 `WAVE_TIER_TABLE.hpT/dmgT` 与 `reqLevel` 两处，别动耦合/出怪时间轴。
