...
// 土系范围技能半径上限 = 陷坑原初始半径(80) × 2/3；与 game.js EARTH_MAX_AOE_RADIUS 同步（v1.1.32 仅土系）
const EARTH_MAX_AOE_RADIUS = 53;
...
function applyEarthSpikeHitSim(spike, isFirstTick) {
  ...
  if (isFirstTick) {
    if (m.quakeChance > 0 && Math.random() < m.quakeChance) {
      const qr = Math.min(EARTH_MAX_AOE_RADIUS, Math.max(40, 60 + m.quakeRadius));
      for (const z of zombies) {
        if (z.health <= 1) continue;
